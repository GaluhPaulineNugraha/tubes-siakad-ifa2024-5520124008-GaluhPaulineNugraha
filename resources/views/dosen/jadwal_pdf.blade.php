...
<body>
    ...
    <div class="info-section">
        <table class="info-table">
            ...
            <tr><td>Total Mata Kuliah</td><td>: <strong>{{ $jadwal->count() }}</strong></td></tr>
        </table>
    </div>
    ...
    <table class="data-table">
        ...
        <tbody>
            ...
            <tr>
                <td>{{ $index + 1 }}</td>
                <td style="text-align: left;">{{ $j->matakuliah->nama_matakuliah }}</td>
                <td><span class="badge">{{ $j->hari }}</span></td>
                <td>{{ date('H:i', strtotime($j->jam)) }} WIB</td>
                <td>{{ $j->kelas }}</td>
            </tr>
            @empty
            <tr><td colspan="5" style="text-align:center;padding:30px;color:#999;">Belum ada jadwal mengajar</td></tr>
            @endforelse
        </tbody>
    </table>
